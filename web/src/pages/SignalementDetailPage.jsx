...
const SignalementDetailPage = () => {
  ...
  if (!signalement) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: 'var(--bg-page)' }}>
        <div style={{ color: 'var(--text-main)' }}>Signalement introuvable</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-20" style={{ backgroundColor: 'var(--bg-page)' }}>
      ...
      <div className="max-w-[430px] mx-auto px-4 pt-20">
        ...
        >
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="font-medium" style={{ color: 'var(--text-main)' }}>{signalement.categorie}</p>
            </div>
            <div>
              <p className="text-sm mb-1" style={{ color: 'var(--text-sub)' }}>Adresse</p>
              <p style={{ color: 'var(--text-main)' }}>{signalement.adresse}</p>
            </div>
            <div>
              <p className="text-sm mb-1" style={{ color: 'var(--text-sub)' }}>Date</p>
              <p style={{ color: 'var(--text-main)' }}>{formatDate(signalement.created_at)}</p>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}
        >
          <h2 className="font-bold text-lg mb-3" style={{ color: 'var(--text-main)' }}>Description</h2>
          <p className="leading-relaxed" style={{ color: 'var(--text-main)' }}>{signalement.description}</p>
        </div>
        ...
        <div
          ...
          style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', boxShadow: 'var(--shadow)' }}
        >
          <h2 className="font-bold text-lg mb-4" style={{ color: 'var(--text-main)' }}>Suivi</h2>
          <div>
